<template>
  <MainContainer title="Solicitudes Ministerio del Trabajo">
    <div style="border-radius: 12px; background-color: #ffff; margin-bottom: 20px">
    ...
    <!-- Filtros -->
    <div class="container_background" style="padding: 1rem" v-if="showDetailSections">
    ...
    <q-dialog v-model="showTrazabilidad">
      <q-card style="padding: 25px 40px; border-radius: 20px; max-width: 100%; position: relative">
        <q-btn
          icon="close"
          flat
          round
...
const allRowsAsignaciones = [
  {
    ...
    trazabilidad: h('span', { class: 'material-icons text-orange text-h4' }, 'history'),
...
const showDetailSections = computed(() => !!selectedFilter.value || !!selectedFilter2.value);
...
const rowsAsignaciones = computed(() => {
  if (selectedFilter2.value) {
    return allRowsAsignaciones.filter((row) => row.estado === selectedFilter2.value?.label);
  }
...
const paginationAsignacionesTotal = computed(() =>
  Math.ceil(rowsAsignaciones.value.length / paginationAsignaciones.value.rowsPerPage),
);
const onPaginationAsignaciones = (val: any) => {
  paginationAsignaciones.value = val;
  const total = Math.ceil(rowsAsignaciones.value.length / paginationAsignaciones.value.rowsPerPage);
};
