<template>
            ...
            <div class="tw-grid tw-grid-cols-6 tw-gap-x-4">
              ...
              <div class="tw-col-span-2 tw-mt-4">
                ...
                <q-select :options="filteredSubSeries" map-options emit-value outlined v-model="formData.subSeries"
                  ...
                  hide-selected :readonly="expedienteInit">
                  <template v-slot:no-option>
                    <q-item>
                      <q-item-section class="text-grey">
                        Sin resultados
  ...
  <CrearExpediente ref="crearComp" v-if="crearExp" :tutelas-data="props.tutelasData" @is-exp="isCrearExpediente" :is-masivo="props.isMasivo"/>
...
const emit = defineEmits(['update:isExpediente'])
...
const isExpe = ref(props?.isExpediente)
...
const loadExpediente = async () => {
  const { data: expediente } = await sgdeaAxios.get("/expediente/children", {
    // @ts-ignore
    params: { nodeId: props.tutelasData.nodeId },
  })
    ...
    if (status === 200) {
      ...
      await crearTrazabilidad({
          ...
          descripcion:
            ...
            `Comentario: Se ha incluido expediente Expediente:${data.titulo}`
            ,
          comentario: "Se ha incluido expediente a la comunicación.",
          nombre: auth.$state.userInfo.name,
          tramite: "Comunicación Masiva",
          accion: `${auth.$state.userInfo.name} ha incluido expediente a la comunicación ${props.tutelasData.idRadicado}`,
        });
      isExpe.value = true
