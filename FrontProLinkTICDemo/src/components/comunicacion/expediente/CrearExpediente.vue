...
import { onMounted, Ref, ref, watch, defineAsyncComponent } from "vue";
...
const CommentTextArea = defineAsyncComponent(() => import('src/shared/components/TextArea/Comments.vue'));
...
const submit = async () => {
  ...
  try {
    ...
    const { data,status } = await sgdeaAxios.post(`/expediente/newpdate`, sendData)

    if (status === 200) {
      toast.success("Datos guardados exitosamente")
      await crearTrazabilidad({
          proceso: `${props.tutelasData.isMasivo || props.isMasivo? "CORMAS" : "COMIND"}${props.tutelasData.id}`,
        secuencia: props.tutelasData.idRadicado,
        // estado: `${props.tutelasData.isMasivo ? "En edicion" : "CREADA"}${props.tutelasData.id}`,
        estado: "En Edición",
        descripcion:
          `Usuario: ${auth.$state.userInfo.name}` +
          `Rol: ${auth.$state.userInfo.role} ` +
            `Comentario: Se ha creado un expediente Expediente:${data.titulo}`
            ,
          comentario: "Se ha creado expediente a la comunicación.",
          nombre: auth.$state.userInfo.name,
          tramite: "Comunicación Masiva",
          accion: `${auth.$state.userInfo.name} ha creado un expediente a la comunicación ${props.tutelasData.idRadicado}`,
        });
      emit('isExp')
      success.value = true
    }
