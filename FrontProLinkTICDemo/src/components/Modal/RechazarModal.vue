<template>
  <q-dialog v-bind:model-value="showModal" persistent class="modal-custom-dialog">
    <q-card class="tw-p-[40px]   tw-w-[800px] tw-max-w-[800px] ">
      ...
      <q-card-section v-if="props.showGestionador" class="tw-px-0 tw-pt-0">
      ...
      <q-form @submit.prevent="onSubmit" @validation-error="toast.error('Debe diligenciar los campos obligatorios para continuar con el proceso')">
        <label class="tw-text-[16px] tw-text-label tw-text-modal-grey-subtitle tw-font-bold tw-ml-2"> {{ props.text }} </label>
        <q-input
          v-bind="$attrs"
          type="textarea"
          label="Inserte"
          class="font-normal  tw-text-[18px] tw-w-full modal-custom-input tw-rounded-[28px] "
          outlined
          dense
          :rules="[isSuccess, (val) => maxLengthInput(props.motivoLimit, val)]"
