<template>
  <div>
    ...
    <div class="tw-flex tw-justify-between tw-items-center tw-pt-4 tw-mb-8">
      <h2 class="tw-font-bold tw-text-4xl tw-py-4">Ubicación de expedientes</h2>
    </div>
    ...
    <div class="tw-gap-4">
      ...
      <q-card class="q-mt-lg tw-p-7 tw-rounded-lg" flat>
        <div class="col row tw-flex tw-justify-between tw-items-center tw-pb-4">
          ...
          <q-btn class="tw-bg-secondary tw-rounded-xl" text-color="white" @click="$router.push('/inventarioFisico/ubicacion/asignar')"
            v-if="rowsSelected.length">
            Asignar Ubicación
          </q-btn>
        </div>

        <Table :TABLE_HEADER="EXPEDIENTE_HEADER" :TABLE_BODY="dataTable" :selection="undefined"
          msg-no-data="No se encontraron datos con los criterios ingresados. Por favor verifique e intente nuevamente."
          key-id="id" icon="visibility" v-model:item-selected="rowsSelected" />
      </q-card>
    </div>
  </div>
</template>
...
const lengthValidation = (n: number, val: string) => {
  return val.length <= n ? "" : "Máximo " + n + " cáracteres"
};
