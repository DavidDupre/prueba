<template>
    <div class="q-my-md">
      <div class="tw-flex tw-justify-between tw-items-center">
        <span class="tw-font-bold tw-text-3xl tw-my-5"> Devolución de envíos - Correo eléctronico y correo electrónico certificado</span>
      </div>
      ...
      <q-card class="tw-rounded-xl q-mt-lg tw-p-5" flat bordered>
        <div class="flex items-center justify-between q-px-md q-mb-sm row col-12">
          <span class="text-weight-bold text-h6" style="color: #2c3948">Resultado de la Búsqueda</span>
  ...
  const pageSize = ref(100000) //FIXME:
  ...
  const REPORTES_HEADER = [
    ...
    },
    {
      name: "gestionador",
      label: "Gestionador",
      field: (row) => row.userId?.firstname + ' ' + row.userId?.lastname ?? 'N/A',
      sortable: true,
      align: 'center'
    },
    {
  ...
  async function getData() {
    ...
    if (params.some(elemento => elemento.startsWith('numeroIdentificacion'))) {
      const numeroIdentificacion = filteredDataTable.value.find(it => it.informacionProceso?.numeroDocumentoAccionante == filtro.value.identificacion)
      const codigo = numeroIdentificacion?.codigoProceso
      const index = params.findIndex(it => it.startsWith('numeroIdentificacion'))
      params[index] = `numeroIdentificacion=${codigo}`
    }
