<template>
  ...
  <Modal v-model:model-value="showModal" title="¡Espera un momento!"
    ...
    :is-success="false" textButtonCancel="No" cancel-button @handleAccept="showModal = false; router.push('/bandeja');
...
const loadUsers = async (initial: boolean = false) => {
  try {
    if (initial) {
      const officeName = listadoOficinas.value.find((e) => e.value === auth.$state.userInfo.oficina)?.label
...
const handleAcceptModal = async () => {
  try {
    const response = await sgdeaAxios.post(`/medicina/asignar/${props.mlData.id}`, {
