  ...
  <div>
    <q-table flat :rows="TABLE_BODY" :columns="TABLE_HEADER" :row-key="keyId" table-header-class="text-black text-h6 "
      ...
      <template v-slot:body-cell-formato="props">
        <q-td v-if="['application/pdf', 'pdf'].includes(obtenerExtension(props.row.nombre.toLowerCase()))"
          class="tw-mx-auto">
          <center>
        ...
        <q-td
          v-else-if="['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'docx', 'application/msword', 'doc'].includes(obtenerExtension(props.row.nombre.toLowerCase()))"
          class="tw-mx-auto">
          <center>
        ...
        <q-td
          v-else-if="['application/vnd.ms-excel', 'xls', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'xlsx', 'xml'].includes(obtenerExtension(props.row.nombre.toLowerCase()))"
          class="tw-mx-auto">
          <center>
      ...
      <template v-slot:body-cell-acciones="props">
        <q-td class="text-center">
          <q-btn size="sm" color="secondary" :icon="icon" flat @click="
            editRow(props.row.id_seriesubserie || props.row.id || props.row[rowId])" />
      ...
      <template v-slot:body-cell-actions="props">
        <q-td class="text-center">
          ...
          <q-btn v-if="['application/pdf', 'pdf'].includes(obtenerExtension(props.row.nombre.toLowerCase()))" size="sm"
            color="primary" :icon="thirdIcon" flat @click="handleThirdIcon(props.row)" />
        </q-td>
      </template>
...
const editData = (rowEditData: any) => {
  emit("handleEditData", rowEditData);
};
...
const handleThirdIcon = (rowPrint: any) => {
  emit("handleThirdIcon", rowPrint);
};
