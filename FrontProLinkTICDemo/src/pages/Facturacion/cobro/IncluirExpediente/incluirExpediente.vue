...
const loadDependencias = async () => {
  try {
    ...
    // formData.value.oficina = listadoOficinas.value;
  } catch (error) {
...
const loadExpedientes = async () => {
  ...
  const response = await sgdeaAxios.get(`/expediente`, {
    params: {
      idSubserie: formData.value.subSeries || formData.value.subSeries.value,
      modulo: 6,
    },
  });
...
const submit = async () => {
  if (await form.value.validate()) {
    ...
    try {
      const sendData = {
        // @ts-ignore
        dependencia: formData.value.oficina.value,
        // @ts-expect-error
