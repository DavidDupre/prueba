<template>
  <div>
    <q-table flat :rows="props.DOCUMENTOS_BODY" :columns="DOCUMENTOS_HEADER" row-key="nombreDocumento"
      ...
      no-results-label="No se encontraron resultados coincidentes con la búsqueda." v-model:pagination="pagination"
      hide-pagination>

      <template v-slot:body-cell-formato="props">
        <q-td v-if="['pdf', 'application/pdf'].includes(props.row.formato?.toLowerCase())"
          class="tw-mx-auto tw-cursor-pointer">
          <center>
            <img @click="visualize(props.row.id)" v-lazy :data-url="pdf" alt="pdf" width="30px" />
          </center>
        </q-td>
        <q-td
          v-else-if="['docx', 'doc', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/msword'].includes(props.row.formato?.toLowerCase())"
          class="tw-mx-auto">
          <center>
        ...
        <q-td
          v-else-if="['xls', 'xlsx', 'csv', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel'].includes(props.row.formato?.toLowerCase())"
          class="tw-mx-auto">
          <center>
        ...
        <q-td v-else-if="['jpg', 'png', 'jpeg'].includes(props.row.formato?.toLowerCase())" class="tw-mx-auto">
          <center>
        ...
        <q-td v-else-if="['txt', 'mbp'].includes(props.row.formato?.toLowerCase())" class="tw-mx-auto">
          <center>
        ...
        <q-td v-else-if="props.row.formato?.toLowerCase() == 'msg'" class="tw-mx-auto">
          <center>
...
const pagesNumber = computed(() => Math.ceil((props.DOCUMENTOS_BODY?.length ?? DOCUMENTOS_BODY.length) / pagination.value.rowsPerPage))
