...
async function loadData() {
  try {
    const response = await sgdeaAxios.get(
      "/trazabilidad/obtenerPorCodProceso/" + "CTO" + props.cobroData.idCto
    );
...
async function addComment() {
  try {
    await crearTrazabilidad({
      proceso: "CTO" + props.cobroData.idCto,
      secuencia: props.cobroData.sec,
      estado: props.cobroData.estado.toLowerCase(),
      descripcion: "Inserto comentario al trámite",
      comentario: comment.value,
      nombre: auth.$state.userInfo.name,
      tramite: props.cobroData.sec,
