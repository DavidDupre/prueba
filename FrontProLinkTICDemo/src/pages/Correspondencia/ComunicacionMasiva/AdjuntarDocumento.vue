...
const getDocuments = async () => {
  ...
  const response = await sgdeaAxios.get<any>(
    `/correspondencia/obtenerDocs?idComunicacionMasivaId=${route.params.id }`
  );
...
const deleteDoc = async (id, nodeId) => {
  try {
    ...
    const response = await sgdeaAxios.delete(
      `/correspondencia/deleteFileComunicacionMasiva/${nodeId}/${id}`
    );
...
const handleFileUpload = async () => {
  if (await formFile.value.validate()) {
    ...
    formdata.append("descripcionRadicado", "SALIDA");

    try {
      const response = await sgdeaAxios.post(
        `api/v1/integracion/alfresco/agregarAnexos/ComunicacionMasiva?comunicacionMasivaId=${props.detailsRadicado.id}&usuarioId=${auth.$state.userInfo.userid}&es_devolucion=false`,
        formdata,
        {
          headers: {
...
const trazabilidad = async (type) => {
  const acciones = {
    [AccionesDocumentos.ELIMINAR_DOCS]: {
      descripcion: `Usuario:${auth.userInfo.name} Comentario: Se eliminó un documento de soporte a la comunicación`,
      comentario: "Se eliminó un documento de soporte del radicado",
      accion: `Usuario ${auth.userInfo.name} elimino documentos de soporte a la comunicación`,
    },
    [AccionesDocumentos.AGREGAR_DOCS]: {
      descripcion: `Usuario: ${auth.userInfo.name} Comentario: Se adjuntaron documentos a la comunicación`,
      comentario: "Se agregaron documento(s) de soporte al radicado",
      accion: `Usuario ${auth.userInfo.name} ha adjuntado documentos a la comunicación`,
    },
  };
