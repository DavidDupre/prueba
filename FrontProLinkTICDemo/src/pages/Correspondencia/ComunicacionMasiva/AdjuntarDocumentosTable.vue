    ...
    <div>
      <q-table flat :rows="TABLE_BODY" :columns="TABLE_HEADER" :row-key="keyId" table-header-class="text-black text-h6 "
          ...
          <q-td class="tw-mx-auto row !tw-h-[70px] tw-flex tw-justify-start tw-gap-x-1 tw-items-center">
            <div class="row">
              <q-avatar class="tw-w-[50px] tw-h-[50px]">
                <img v-lazy :data-url="img" alt="image">
              </q-avatar>
            </div>
        ...
        <template v-slot:body-cell-formato="props">
          ...
          <q-td v-else-if="['application/vnd.ms-excel', 'xls', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'xlsx'].includes(obtenerExtension(props.row.nombre.toLowerCase()))"
            class="tw-mx-auto">
            <center>
              <img v-lazy :data-url="excel" alt="excel" />
            </center>
          </q-td>
          <q-td v-else-if="['xml'].includes(obtenerExtension(props.row.nombre.toLowerCase()))" class="tw-mx-auto">
          <center>
            <img v-lazy :data-url="xml" alt="xml" width="30px" />
          </center>
        </q-td>
        <q-td v-else-if="['ppt', 'pptx'].includes(obtenerExtension(props.row.nombre.toLowerCase()))" class="tw-mx-auto">
          <center>
            <img v-lazy :data-url="ppt" alt="ppt" width="30px" />
          </center>
        </q-td>
        <q-td v-else-if="['jpg', 'png', 'jpeg', 'webp'].includes(obtenerExtension(props.row.nombre.toLowerCase()))" class="tw-mx-auto"  >
          <center>
            <img v-lazy :data-url="img" alt="img" width="30px"/>
          </center>
        </q-td>
        <q-td v-else-if="obtenerExtension(props.row.nombre.toLowerCase()) == 'mp3' " class="tw-mx-auto"  >
          <center>
            <img v-lazy :data-url="music" alt="mp3" width="30px" />
          </center>
        </q-td>
        <q-td v-else-if="['avi', 'mp4' ].includes(obtenerExtension(props.row.nombre.toLowerCase()))" class="tw-mx-auto"  >
          <center>
            <img v-lazy :data-url="video" alt="video" width="30px" />
          </center>
        </q-td>
        <q-td v-else-if="['txt', 'bmp' ].includes(obtenerExtension(props.row.nombre.toLowerCase()))" class="tw-mx-auto"  >
          <center>
            <img v-lazy :data-url="txt" alt="txt" width="30px" />
          </center>
        </q-td>
        <q-td v-else-if="['zip', 'rar' ].includes(obtenerExtension(props.row.nombre.toLowerCase()))" class="tw-mx-auto"  >
          <center>
            <img v-lazy :data-url="zip" alt="zip" width="30px" />
          </center>
        </q-td>
        <q-td v-else-if="obtenerExtension(props.row.nombre.toLowerCase()) == 'msg' || obtenerExtension(props.row.nombre.toLowerCase()) == 'eml'" class="tw-mx-auto"  >
          <center>
        ...
        <template v-slot:body-cell-actions="props">
          <q-td class="text-center">
            ...
            <q-btn size="sm" color="primary" :icon="secondIcon" flat
              @click="handleSecondIcon(props.row.id, props.row.nodeId)" />
          </q-td>
        </template>
