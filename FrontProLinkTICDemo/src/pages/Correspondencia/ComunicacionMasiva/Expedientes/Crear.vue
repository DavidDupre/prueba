        ...
        <section class="tw-px-8 tw-pt-3">
          <div class="tw-grid tw-grid-cols-4 tw-gap-x-4">
            <div class="tw-col-span-2">
              ...
              <q-select map-options emit-value outlined v-model="formData.oficina" label="Inserte" dense disabled />
            </div>
            <div class="tw-col-span-2">
...
onMounted(async () => {
  ...
  formData.value.numeroDocumento = props.corData.afectado?.numeroDocumento ?? 'NA';
  formData.value.numeroIdentificacion = props.corData.caseId ? props.corData.caseId : String(Math.round(Math.random() * 1000)); ;
  await getData()
  await loadUsuarios()
});
...
const getData = async () => {
  try {
    // Oficinas
    const responseOficinas = await sgdeaAxios.get(`oficinas/all`)
    const oficina = await responseOficinas.data.find(it => it.nombre === auth.$state.userInfo?.relations[0].oficina);
    const idOficina = responseOficinas.data.find((e) => e.nombre == oficina.nombre)?.id;
    filteredOficinas.value = { label: oficina.nombre, value: idOficina }
