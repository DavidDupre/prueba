<template>
  ...
  import CrearExpediente from "./Crear.vue";
  ...
  const compararPorPropiedad = (a, b) => {
    ...
    const propiedad = "label"; // Cambia "nombre" por la propiedad deseada

    if (a[propiedad].toLowerCase() < b[propiedad].toLowerCase()) {
        return -1;
    } else if (a[propiedad].toLowerCase() > b[propiedad].toLowerCase()) {
        return 1;
    } else {
        return 0;
    }
  ...
  const loadTipoDocumental = async (id) => {
    ...
    listadoTiposDocumentales.value.sort(compararPorPropiedad)
