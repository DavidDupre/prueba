  ...
  <q-page class="q-pa-md">
    ...
    <div class="tw-flex tw-items-center tw-justify-between">
      <div class="tw-flex tw-flex-col">
        <ComponentTitle
          class="q-mt-lg"
          :title="`Comunicación #${allInfo?.idRadicado}`"
    ...
    <q-tab-panels v-model="tab" animated class="tw-bg-transparent">
      ...
      <q-tab-panel name="documento-salida" class="q-pa-none">
        <q-card class="q-mt-lg tw-rounded-xl q-pa-lg" flat>
          <Editor
            :data="dataFooter"
            :referentKey="datosSalida['destinatario']"
            :restric="restric"
      ...
      <q-tab-panel name="datos-salida" class="q-pa-none">
        <q-form ref="myForm10" v-if="stepDataSend === 0" greedy>
          <q-card class="q-mt-lg tw-rounded-xl q-pa-lg" flat>
            ...
            <FormFormDatosSalida
              ...
              :tipo-comunicacion="allInfo.tipoComunicacion?.id"
            />
          </q-card>
        </q-form>
        ...
        <q-form ref="myForm20" v-if="stepDataSend === 1" greedy>
          ...
          <div
            v-if="formExitData.copyType === 1 || formExitData.copyType === 4"
          >
            <q-card class="q-mt-lg tw-rounded-xl q-pa-lg" flat>
          ...
          <div
            v-if="formExitData.copyType === 2 || formExitData.copyType === 4"
          >
            <q-card class="q-mt-lg tw-rounded-xl q-pa-lg" flat>
        ...
        <!-- <q-form ref="myForm30" v-if="stepDataSend === 2" greedy>
          <FormEditor :dataInitial="formExitData" />
        </q-form> -->
      </q-tab-panel>
            ...
            <div class="row q-col-gutter-md q-pb-md q-px-md">
              ...
              <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                <span class="tw-w-full">
                  ...
                  <q-input
                    v-model.number="formGeneralData.numeroDocumento"
                    dense
                    outlined
                    label="Inserte"
                    class="tw-rounded-lg"
    ...
    <div
      v-if="tab !== 'datos-salida' && tab !== 'incluir-exp'"
...
const getAllData = async () => {
  ...
  const promises = route.query.id
    ? [getCopiesTypesOptions(), getAllDataComunicacionCo(idComunicacion.value)]
    : [getCopiesTypesOptions()];
  ...
  formGeneralData.value.asunto = allInfo?.value?.datosGenerales?.asunto;
  formGeneralData.value.nombreAfectado =
    allInfo?.value?.datosGenerales?.nombreAfectado;
  formGeneralData.value.numeroDocumento =
    allInfo?.value?.datosGenerales?.numeroDocumentoAfectado;
  formGeneralData.value.tipoDocumento =
    allInfo?.value?.datosGenerales?.tipoDocumentoAfectado?.nombre;

  formExitData.value.datosSalidaFormaEnvioId =
    allInfo?.value?.datosSalida?.formaEnvio?.id;
  formExitData.value.datosSalidaTipoDocumentoDestinatarioId =
    allInfo?.value?.datosSalida?.tipoDocumentoDestinatario;
  formExitData.value.datosSalidaNumeroDocumentoDestinatario =
    allInfo?.value?.datosSalida?.numeroDocumentoDestinatario;
  formExitData.value.datosSalidaNombreDestinatario =
    allInfo?.value?.datosSalida?.nombreDestinatario;
  formExitData.value.datosSalidaDeptoId =
    allInfo?.value?.datosSalida?.departamento;
  const municipio = allInfo?.value?.datosSalida?.municipio;
  formExitData.value.datosSalidaMunicipioId = municipio?.idMunicipio
    ? municipio
    : null;

  formExitData.value.datosSalidaTelefono =
    allInfo?.value?.datosSalida?.telefonoDestinatario;
  formExitData.value.datosSalidaCorreo =
    allInfo?.value?.datosSalida?.correoDestinatario;
  formExitData.value.datosSalidaDireccion =
    allInfo?.value?.datosSalida?.direccionDestinatario;
  formExitData.value.datosSalidaAnexos = allInfo?.value?.datosSalida?.anexos;
  formExitData.value.observacion = allInfo?.value?.datosSalida?.observacion;
  formExitData.value.datosSalidaNumeroDocumentoAfectado =
    allInfo?.value?.datosGenerales?.nombreAfectado;
  formExitData.value.copyType = allInfo?.value?.datosSalida?.tipoCopia?.nombre;
  isDisabled.value =
  ...
  await handleNewCommunicationInfo({
    nombreComunicacion: allInfo.value?.nombre,
    prefijoComunicacion: allInfo.value?.prefijoComunicacion?.id,
    tipoComunicacion: allInfo.value?.tipoComunicacion?.id,
    tipoPlantilla: allInfo.value?.tipoPlantilla?.id,
  });
  ...
  const gestionadorOficinaAsunto =
    allDataLoaded?.status === "fulfilled" &&
...
watch(
  ...
  (value) => {
    ...
    dataFooter = {
      ...
      elaboro: elaboroUsuario,
      aprobo: "",
      reviso: "",
      formaEnvio: datosSalida.value.destinatario.shippingMethod,
...
onMounted(async () => {
  ...
  idRadicado.value = route.query.id;
  await getInfoCombos();
  await getData();
  await getAllData();
  await getUserInfo();
...
const getTrazabilidad = async () => {
  const response = await sgdeaAxios.get(
    "/trazabilidad/obtenerPorCodProceso/" + "COMIND" + allInfo.value.id
  );
...
const addFormDataFiles = async () => {
  ...
  if (hardcoreData.value.nodeId) {
    ...
    if (index > 0) {
      ...
      const response = await sgdeaAxios.post(
        `/correspondencia/comunicacion/documentos/${idComunicacion.value}?user=${auth.$state.userInfo.userid}&tipoDocumento=ENTRADA`,
        formDataFiles
      );
...
const getDocs = async () => {
  const { data } = await sgdeaAxios.get(
    `/correspondencia/comunicacion/documentos/ENTRADA/${idComunicacion.value}`
  );
...
const handleSendAprobacion = async () => {
  ...
  try {
    ...
    await sgdeaAxios.post(
      `correspondencia/comunicacion/asignarAprobacion/${idRadicacion.value}`,
      {
...
const handleSubmitEliminarRadicacion = async () => {
  if (dataComunicacion.value.data.id === null) {
    toast.error(objResponse.no_data);
    return;
  }
  try {
    await sgdeaAxios.put(
      `correspondencia/comunicacion/actualizarEstado/${dataComunicacion.value.data.id}?idNuevoEstado=2`
    );
...
function formatCopyInterna(data) {
  return ` ${data.correspondenciaDependencia?.nombre ?? ""} - ${
    data.correspondenciaFuncionario?.fullname ?? ""
  }`;
}
function formatCopyExterna(data) {
  return ` ${data.formaEnvio.nombre + " - "} ${
    data.tipoDocumentoDestinatario.nombre ?? "" + " - "
  }  ${data.nombreDestinatario + " - "} ${
    data.direccionDestinatario ? data.direccionDestinatario + " - " : ""
  } ${
    data.paisDestinatario.nombre ? data.paisDestinatario.nombre + " - " : ""
  } ${
    data.departamentoDestinatario
    ...
    ${data.telefonoDestinatario ? data.telefonoDestinatario + " - " : ""}  ${
