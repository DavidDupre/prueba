          ...
          <div class="tw-grid tw-grid-cols-4 tw-gap-x-4">
            ...
            <div class="tw-col-span-2 tw-mt-4">
              ...
              <q-input type="text" v-model="formData.numeroDocumento" label="Inserte" dense outlined
                :rules="[v => maxLengthInput(30, v)]" @keypress="onlyAlphaNumeric" />
            </div>
            <div class="tw-col-span-2 tw-mt-4">
...
import { onMounted, Ref, ref, watch, defineExpose } from "vue";
...
onMounted(async () => {
  ...
  formData.value.numeroDocumento = props.corData.informacionProceso.numeroDocumentoAfectado;
  formData.value.numeroIdentificacion = props.corData.codigoProceso;
  formData.value.expedientes = `EXP-${props.corData.codigoProceso}-${formData.value.numeroDocumento == 'N/A' ? 'NA' : formData.value.numeroDocumento}`
  await getData()
});
...
const loadDependencias = async () => {
  try {
    ...
    filteredDependencias.value = response.data.map((e) => {
      return {
        label: e.nombre,
    ...
    formData.value.dependencia = filteredDependencias.value.find((e) => e.value === auth.userInfo.dependencia)
  } catch (error) {
...
const submit = async () => {
  ...
  try {
    const sendData = {
      ...
      modulo: 8,
      radicado: props.corData.idRadicado,
    }
    ...
    if (status === 200) {
      ...
      await crearTrazabilidad({
        proceso: `COR${props.corData.id}`,
        secuencia: props.corData.idRadicado,
        estado: props.corData.estadoProceso.estado,
        ...
        nombre: auth.userInfo.name,
        tramite: props.corData.tipoTramite.nombre,
        accion: `Se Incluyo un expediente al tramite`
      })
      emit('isExp')
      success.value = true
    }
