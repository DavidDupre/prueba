...
const confirmAsignar = async () => {
  try {
    ...
    if (cardsAsignarAprobador.value.length === 0) {
    ...
    if (responseAprobador.status === 200) {

      await crearTrazabilidad({
        proceso: `COR${id}`,
        secuencia: radicado.idRadicado,
        estado: "Por aprobar",
        descripcion: `Usuario: ${userInfo.value.name} - ${userInfo.value.role} Aprobador: ${cardsAsignarAprobador.value[0]?.name }`,
        comentario: 'Aprobacion',
        nombre: userInfo.value.name,
        tramite: radicado.tipoTramite.nombre,
        accion: `El trámite ha enviado a aprobar`
      });
