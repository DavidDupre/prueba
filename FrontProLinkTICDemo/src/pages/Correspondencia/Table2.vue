<template>
  ...
  <q-table flat :rows="filteredData" :columns="TABLE_HEADER" :row-key="keyId" table-header-class="text-black text-h6"
    ...
    v-model:selected="selection" :loading="isLoading" :table-style="'table-layout: fixed; width: 100%;'"
    ...
    <template v-slot:body-cell-nombreAfectado="props">
      <q-td style=" min-width: 20vw; padding: 6px;"  class="text-center">
        <div
          style="
        display: block;
        width: 100%;
      ...
      "
        >
          {{ props.row.documentoAfectado && props.row.nombreAfectado
          ? `${props.row.nombreAfectado} - ${props.row.documentoAfectado}`
          : '-' }}
        </div>
      </q-td>
    </template>
    ...
    <template v-slot:body-cell-nombreRemitente="props">
      <q-td style=" min-width: 20vw; padding: 6px;"  class="text-center">
        <div
          style="
        display: block;
      ...
      "
        >
          {{ props.row.documentoRemitente && props.row.nombreRemitente
          ? `${props.row.nombreRemitente} - ${props.row.documentoRemitente} `
          : '-' }}
        </div>
      </q-td>
    </template>
        ...
        <div class="tw-flex tw-row">
          ...
          <q-btn class="tw-w-24 tw-h-9" flat @click="editData(props.row.id)">
            <q-icon v-if="
              ...
              props.row.estado?.toLowerCase() === 'por revisar' ||
              props.row.estado?.toLowerCase() === 'anulado' &&
              props.row.fechaAnulacion !== null &&
              props.row.anulacionConReinicio === false" color="primary" name="visibility" @click="but(props.row)" />
      ...
      <div class="row items-center q-mt-md tw-w-full">
        <div class="col-auto tw-flex tw-gap-x-3 tw-items-center">
          ...
          <q-select style="width: 120px; height: 40px" outlined dense mapOptions emitValue
            :options="[5, 20, 25, 50, 100]" v-model:model-value="rowsPerPage" />
        </div>

        <div class="col tw-flex tw-justify-center">
          <q-pagination v-model="numPage" color="primary" :max="totalPages" size="md" ellipses max-pages="10"
