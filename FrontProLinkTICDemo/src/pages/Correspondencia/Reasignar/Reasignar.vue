      ...
      <div class="tw-bg-white q-mt-lg tw-rounded-md">
        <div class="full-width q-px-lg q-pb-lg alineation tw-gap-3 tw-flex tw-flex-row">
          <div class="width-container tw-w-1/2 q-px-sm tw-rounded-xl" style="border: 1px solid #E3E4E5;">
              <q-table flat :rows="listUsersOriginal" @update:selected="handleAssign" :columns="columnsAsignar" row-key="id"
                ...
                <template v-slot:top>
                  ...
                  <div class="row tw-justify-center tw-w-full">
                    ...
                    <q-select v-model="selectOficina" outlined dense fill-input :options="oficinas" class="tw-w-1/2 tw-overflow-hidden tw-text-nowrap tw-text-ellipsis" label="Seleccione" style="padding: 0px;" />
                    <q-select :options="searchHelpDataFiltered" fill-input hide-selected
                      use-input v-model="searchUsuariosList" @filter="selectOptionsFilterFn" class="tw-w-1/2" outlined dense
    ...
    <div class="tw-bg-white q-mt-lg" style="border-radius: 5px">
      ...
      <q-form ref="formRef">
        ...
        <div class="row q-pt-lg q-pb-xl justify-center tw-gap-x-5">
          ...
          <q-btn class="tw-w-1/4 tw-h-11 bg-primary text-white" @click="confirmAsignar">Reasignar</q-btn>
        </div>
      </q-form>
    </div>
...
watch(
    ...
    async(newValue, oldValue) => {
      if (newValue !== oldValue) {
        ...
        // @ts-ignore
        const { data: gestionadores } = await sgdeaAxios.get(`/api/usuarios/rolYoficina?rolName=Gestionador${ newValue.value ? '&oficinaId='+newValue.value : '' }`);
        // @ts-ignore
        const { data: asignadores } = await sgdeaAxios.get(`/api/usuarios/rolYoficina?rolName=Asignador Responsable${ newValue.value ? '&oficinaId='+newValue.value : '' }`);
        ...
        listUsersOriginal.value = [...listUsersOriginal.value, ...gestionadores.map((item) => ({
          colaborador: item.firstname + ' ' + item.lastname,
          enabled: item.enabled,
          id: item.id,
          user: item.userName,
...
async function getAllRegisters() {
  try {
    ...
    const { data: asignadores } = await sgdeaAxios.get(`/api/usuarios/rolYoficina?rolName=Asignador Responsable${ oficinaUser?.id ? '&oficinaId='+oficinaUser?.id : '' }`);
...
const confirmAsignar = async () => {
  if (await formRef.value.validate()) {
    try {
      ...
      const usersGestionadores = selection.value.filter(it => it.cargo === 'Gestionador')
      const validateUserGestor = form.value.usuarios.filter(item => usersGestionadores.some(it => it?.id === item.usuario));

      if (usersGestionadores.length > 0 && validateUserGestor.length > 0) {
        if (validateUserGestor.some(it =>  it.typeTiempo == '' || it.time == null)) {
          toast.error('Debe llenar el tiempo por gestionador');
          return;
        }
      }

      if (usersGestionadores.length > 0 && usuarioConTiempoInvalido.value) {
        toast.error('El tiempo por gestionador debe ser valido');
        return;
      }

      const id = props?.detailsRadicado.id
      let response;
      if (selection.value[0]?.cargo === 'Asignador Responsable') {
        response = await sgdeaAxios.put(`/correspondencia/asignarCorrespondenciaAsignador/${id}`, {
          observaciones: form.value.observaciones,
          usuario: selection.value[0]?.id
        });

      } if (selection.value[0]?.cargo === 'Gestionador') {
