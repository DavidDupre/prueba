<template>
  <section class="tw-flex tw-flex-row">
    <main class="tw-min-h-screen tw-w-full tw-p-7 tw-pt-0">
      ...
      <div class=" tw-pt-4">
        <h1 class="tw-text-3xl tw-font-bold tw-my-4">Crear Usuario</h1>
        <p class="tw-text-[#969EAF] tw-pl-2 tw-pb-6">Registra los nuevos usuarios que accederan a la plataforma.</p>
      </div>
      <q-form ref="formUsers" greedy>
        <q-card class="tw-mt-[20px] tw-rounded-xl q-pa-lg" flat>
          ...
          <div
            ...
            <span class="col">
              ...
              <q-select v-model="form.tipoDoc" label="Seleccione" outlined map-options emit-value class="mb-3"
                :options="opsTipoDoc" dense :rules="[inputRequired]" />
            </span>
            <span class="col">
              <label class="tw-text-label">Número de Documento *</label>
              <q-input v-model="form.numDoc" label="Inserte" outlined class=""
                :rules="[inputRequired, alphanumericAbsolute, (val) => maxLengthInput(15, val)]" dense />
            </span>
          </div>
          <template v-if="!isNitOrRut">
            <div class="row tw-gap-5">
              <span class="col">
                ...
                <q-input v-model="form.firstName" label="Inserte" outlined class=""
                  :rules="[inputRequired, alphanumericAbsolute, (val) => maxLengthInput(50, val)]" dense />
              </span>
              <span class="col">
                <label class="tw-text-label">Segundo Nombre *</label>
                <q-input v-model="form.segName" label="Inserte" outlined class=""
                  :rules="[alphanumericAbsolute, (val) => maxLengthInput(50, val)]" dense />
              </span>
              <span class="col">
                <label class="tw-text-label">Primer Apellido *</label>
                <q-input v-model="form.firstApellido" label="Inserte" outlined class=""
                  :rules="[inputRequired, alphanumericAbsolute, (val) => maxLengthInput(50, val)]" dense />
              </span>
              <span class="col">
              <label class="tw-text-label">Segundo Apellido </label>
              <q-input v-model="form.segApellido" label="Inserte" outlined class=""
                :rules="[alphanumericAbsolute, (val) => maxLengthInput(50, val)]" dense />
              </span>
            </div>
          </template>
          <span v-else class="">
            ...
            <q-input v-model="form.RazonSocial" label="Inserte" outlined class="mb-3"
              :rules="[isNotEntidadRemitente ? inputRequired : null, (val) => maxLengthInput(50, val)]" dense />
          </span>
        </q-card>
          ...
          <div
            ...
            <span class="col">
              ...
              <q-select v-model="form.departamento"
                name="departamento" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="departmentsList" :rules="[isNotEntidadRemitente ? inputRequired : null]" @filter="filterDep" @update:model-value="getMunicipios" >
            ...
            <span class="col">
              ...
              <q-select v-model="form.municipio" name="departamento" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="municipiosList" :rules="[isNotEntidadRemitente ? inputRequired : null]"
                @filter="filterMuni">
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section class="text-grey">
            ...
            <span class="col">
              ...
              <q-input v-model="form.dirrecion" label="Inserte" outlined class=""
                :rules="[isNotEntidadRemitente ? inputRequired : null, alphanumeric, (val) => maxLengthInput(50, val)]" dense />
            </span>
          </div>
          <div class="row tw-gap-5 [&>*]:tw-flex [&>*]:tw-flex-col">
            <span class="col">
              <label class="tw-text-label">Celular {{isNotEntidadRemitente ? '*': ''}}</label>
              <q-input v-model="form.celular" label="Inserte" outlined class=""
                :rules="[isNotEntidadRemitente ? inputRequired : null, mobilePhonNumberCel, (val) => maxLengthInput(10, val)]" dense />
            </span>
            <span class="col">
              <label class="tw-text-label">Correo Personal</label>
              <q-input v-model="form.correoPersonal" label="Inserte" outlined class=""
                :rules="[(val) => maxLengthInput(25, val)]" dense />
            </span>
            <span class="col">
        ...
        <q-card class="tw-mt-[20px] tw-rounded-xl q-pa-lg" flat>
          ...
          <div
            ...
            <span>
              ...
              <q-input v-model="form.username" label="Inserte" outlined class=""
                :rules="[inputRequired, (val) => maxLengthInput(10, val)]" dense />
            </span>
            <span>
              <label class="tw-text-label">Contraseña *</label>
              <q-input v-model="form.password" label="Inserte" outlined :type="isPwd ? 'password' : 'text'" class=""
                :rules="[inputRequired, (val) => maxLengthInput(15, val), v => minLengthInput(8, v)]" dense>
            ...
            <span>
              ...
              <q-select v-model="form.cargo" name="departamento" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="cargosList" :rules="[inputRequired]"
                @filter="filterCargo">
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section class="text-grey">
            ...
            <span>
              ...
              <q-input v-model="form.correoInstitucional" label="Ingrese" outlined class=""
                :rules="[inputRequired]" dense />
              </span>
            <span>
              <label class="tw-text-label">Tipo de Usuario *</label>
              <q-select v-model="form.tipoUser" label="Seleccione" outlined map-options emit-value class=""
                :options="typesUsersOptions" :rules="[inputRequired]" dense />
            </span>
            <span>
              <label class="tw-text-label">Modulo *</label>
              <q-select v-model="form.modulo_id" label="Seleccione" outlined map-options emit-value class="mb-3"
                :options="modulesOptions" :rules="[inputRequired]" dense />
            </span>
            <span>
              <label class="tw-text-label">Entidad Remitente Asociada para Radicación </label>
              <q-select v-model="form.EntRemiAsociadaRadicacion" name="entidadAsociada" clearable outlined dense use-input hide-selected fill-input input-debounce="0" map-options
                label="Seleccione" :options="entidadesRemitentes" :rules="[]"
                @filter="filterEntidadAsociada" @update:model-value="handleEntidadAsociada" >
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section class="text-grey">
                      Sin resultados
        ...
        <q-card class="tw-mt-[20px] tw-rounded-xl q-pa-lg" flat>
          ...
          <div
            ...
            <span>
              ...
              <q-select v-model="form.dependencia" name="dependencias" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="dependenciasOptions" :rules="[]"
            ...
            <span class="">
              ...
              <q-select v-model="form.oficina" name="oficina" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="oficinasOptions" :rules="[]"
            ...
            <span>
              ...
              <q-select v-model="form.puntoRadicacion" name="puntoRadicacion" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="puntosRadicacion" :rules="[]"
            ...
            <span>
              ...
              <q-select v-model="form.rol" name="rol" outlined dense use-input hide-selected fill-input input-debounce="0" emit-value map-options
                label="Seleccione" :options="roleOptions" :rules="[]"
                @filter="filterRol">
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section class="text-grey">
          ...
          <div class="tw-flex tw-justify-center !tw-flex-row tw-gap-2 [&>*]:tw-p-2 tw-w-full tw-col-span-3 tw-mb-2">
            <q-btn label="Limpiar seleccionados" color="accent" textColor="black" class="tw-rounded-lg col-2 tw-w-[17%]"
              @click="clearTable" v-if="selectedItems.length > 0"/>
            <q-btn label="Agregar" color="primary" class="tw-rounded-lg col-2 tw-w-[17%]" type="button"
              @click="() => handleDepsAndOffices()" />
          </div>
          <Table :TABLE_HEADER="cols" :TABLE_BODY="rows" is-selection="multiple" v-model:item-selected="selectedItems" key-id="id"
            class="tw-col-span-3" ref="table" />
        </q-card>
        <q-card class="tw-mt-[20px] tw-rounded-xl q-pa-lg" flat>
          <div class="row tw-flex tw-justify-center tw-gap-2 [&>*]:tw-p-2">
            ...
            <q-btn label="Crear" color="primary" class="tw-rounded-lg col-2" @click="createUser" />
          </div>
        </q-card>
      </q-form>
      <Modal v-model="cancelModal" title="¿Desea cancelar la acción?" :text="``" text-button="Si"
        @handleAccept="() => router.push('/SGDA/Administracion/usuarios')" label="Confirmar" cancelButton
        text-button-cancel="No"
        @close-modal="() => (cancelModal = false)" />
      <Modal v-model="showModal" title="Creación exitosa" :text="`Se ha creado con éxito el usuario ${form.username}.`"
...
import { maxLengthInput, inputRequired, alphanumeric, mobilePhonNumberCel, minLengthInput, alphanumericAbsolute } from "src/helpers/validations";
...
const cols = [
  { name: "dependencia", label: "Dependencia", align: 'center', field: (row: any) => dependenciasOptions.value.find((dependencia) => dependencia.value === row.dependencia)?.label },
  { name: "oficina", label: "Oficina", align: 'center', field: (row: any) => oficinas.value.find((oficina) => oficina.id === row.oficina)?.nombre ?? "Sin oficina" },
  { name: "puntoradicacion", label: "Punto de Radicación", align: 'center', field: (row: any) => puntosRadicacion.value.find((punto) => punto.value === row.puntoRadicacion)?.label ?? "N/A" },
  { name: "rol", label: "Rol", align: 'center', field: (row: any) => roleOptions.value.find((rol) => rol.value === row.rol)?.label },
];
...
const routes = [
  ...
  {
    name: "Crear Usuario",
    to: "/SGDA/administracion/usuarios/crear",
  },
];
...
const createUser = async () => {
  try {
    ...
    const response = await sgdeaAxios.post(
      `/api/usuarios/registro?${queryParameters}`,
      payload
    );
...
const getData = async () => {
  try {
    ...
    if ([200, 201].includes(response.status)) {
      opsRol.value = response.data.map((rol) => ({
        label: rol.nombre,
        value: rol.id,
    ...
    const { data: tipoDocData } = await sgdeaAxios.get<{id: number, codigo: string, descripcion: string}[]>("/usuarios/obtener-tipoDocumentos");
    opsTipoDoc.value = tipoDocData.map((tipo) => ({
    ...
    modulesOptions.value = modulesData.map((module) => ({
      label: module.nombre,
      value: module.id,
    })).sort((a,b) => a.label.localeCompare(b.label));

    const { data } = await sgdeaAxios.get<Dependencia[]>("/seccionSubSeccion");
    opsDepencias.value = data.map((dependencia) => ({
...
const handleDepsAndOffices = () => {
  ...
  if (form.value.oficina !== '') {
    rows.value.push({id: totalRows.value, oficina: Number(form.value.oficina), dependencia: Number(form.value.dependencia), rol: Number(form.value.rol), puntoRadicacion: Number(form.value.puntoRadicacion)});
  } else {
    rows.value.push({id: totalRows.value, oficina: 0, dependencia: Number(form.value.dependencia), rol: Number(form.value.rol), puntoRadicacion: Number(form.value.puntoRadicacion)});
  }
...
watch(() => form.value.dependencia, (newValue) => {
  oficinasOptions.value = [];
  form.value.oficina = "";
  const oficinasFiltered = oficinas.value.filter((oficina) =>
    newValue === oficina?.dependencia?.idSeccionSubSeccion
  );
