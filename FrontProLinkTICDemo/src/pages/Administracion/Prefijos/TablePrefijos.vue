<template>
  <div>
    <q-table flat :rows="TABLE_BODY" :columns="TABLE_HEADER" :row-key="keyId" table-header-class="text-black text-h6"
      ...
      v-model:pagination="pagination" :selection="'multiple'" v-model:selected="selection" :loading="isLoading">
...
const cambiarEstado = async () => {
  ...
  try {
    ...
    await sgdeaAxios.put(`/medicina/actualizar-prefijo/${itemSelectedToChange.value}`, {
      estado: statusItem === 'activar' ? true : false
    })
