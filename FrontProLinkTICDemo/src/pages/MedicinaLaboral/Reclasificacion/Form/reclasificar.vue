<template>
            ...
            <div class="tw-bg-white tw-p-4" style="border-radius: 5px">
              ...
              <div class="row tw-p-2">
                <div class="col-sm-12 tw-p-2" v-if="!showPqrdForm">
                  <p class="tw-text-label tw-font-semibold">Observaciones* {{ formData.descripcion.length }}/5000</p>
  ...
  <Modal v-model:model-value="showModal" title="¡Espera un momento!"
    ...
    text-button="Si" text-button-cancel="No" @close-modal="() => (showModal = false)" />
...
async function initData() {
  try {
    await getTiposPQRD();
    await getOficinas();
    formData.value.tipoPQRD = tiposPQRD.value.find(x => x.label.toLowerCase() == props.detalleTutela.informacionSolicitud.tipologia.toLowerCase()).value;
  } catch (error) {
    await getTiposPQRD();
    await getOficinas();
...
async function save() {
  ...
  const nombretipo=tiposPQRD.value.find(item=> item.value === formData.value.tipoPQRD ).label
  const oficina = oficinas.value.find(item=> item.value === formData.value.oficina ).label
  ...
  body = {
    idMl: props.mlData.id,
    reclasificarTramite: tramiteId,
    oficina: formData.value.oficina || null,
    observacion: formData.value.observacion || formDataPqrd.value.observaciones,
    motivoRechazo: 'string',
    usuario: auth.$state.userInfo.userid,
  }
  ...
  try {
    ...
    if (showPqrdForm.value) {
      ...
      response = await sgdeaAxios.post('/medicina/reclasificar-ml', body);
    }
    ...
    if (response.status === 200) {
      ...
      await crearTrazabilidad({
        ...
        estado: 'Solicitud de reclasificaión',
        descripcion: `Trámite anterior: Medicina Laboral`+`Trámite nuevo: ${nombretipo}`+`Oficina: ${oficina} `,
        comentario: 'Reclasificar',
        nombre: auth.$state.userInfo.name,
        accion: `${auth.$state.userInfo.name} solicitó una reclasificación del caso`,
