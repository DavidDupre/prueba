          ...
          <div class="tw-grid tw-grid-cols-4 tw-gap-x-4">
            <div class="tw-col-span-2">
              ...
              <q-select :options="filteredDependencias" map-options outlined v-model="formData.dependencia"
                ...
                :loading="loadingDependencias" :disable="loadingDependencias"
              />
            </div>
            <div class="tw-col-span-2">
              ...
              <q-select :options="filteredSeries" map-options emit-value outlined v-model="formData.series"
                ...
                hide-selected :loading="loadingSeries" :disable="loadingSeries">
            ...
            <div class="tw-col-span-2 tw-mt-4">
              ...
              <q-select :options="filteredSubSeries" map-options emit-value outlined v-model="formData.subSeries"
                ...
                hide-selected :loading="loadingSubseries" :disable="loadingSubseries">
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section class="text-grey">
                      Sin resultados
            ...
            <div class="tw-col-span-2 tw-mt-4">
              ...
              <q-select :options="filteredTiposDocumentales" map-options emit-value outlined v-model="formData.tipoDocumental"
                label="Inserte" dense :rules="[inputRequired]" @filter="filterTiposDocumental" use-input fill-input
                hide-selected :loading="loadingTiposDoc" :disable="loadingTiposDoc">
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section class="text-grey">
                      Sin resultados
...
const submit = async () => {
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      emit('finally', response.data.nodeId)
      success.value = true
    }
  } catch (error) {
...
watch( () => formData.value.series, async (newValue) => {
  if (newValue) {
...
watch(() => formData.value.subSeries, async (newValue) => {
  if (newValue) {
    await loadTipoDocumental(newValue)
    formData.value.tipoDocumental = '';
    form.value.reset();
