<template>
            ...
            <div class="tw-grid tw-grid-cols-6 tw-gap-x-4">
              <div class="tw-col-span-3">
                ...
                <q-select :options="filteredDependencias" map-options outlined v-model="formData.dependencia"
                  ...
                  :disable="props.isDisabled || expedienteInit" :readonly="expedienteInit" />
              </div>
              <div class="tw-col-span-3">
                ...
                <q-select :options="filteredSeries" map-options emit-value outlined v-model="formData.series"
                  ...
                  :disable="props.isDisabled || expedienteInit">
  ...
  <q-select 
    ...
    label="Inserte" 
    dense 
    :rules="subseriesRequired ? [inputRequired] : []"
    @filter="filterSubSeries" 
    use-input 
    fill-input
    hide-selected 
    :readonly="expedienteInit" 
    :disable="props.isDisabled || expedienteInit || !subseriesRequired"> <!-- ← Deshabilitar si no es requerido -->
    <template v-slot:no-option>
      <q-item>
        <q-item-section class="text-grey">
          Sin resultados
              ...
              <div class="tw-col-span-2 tw-mt-4">
                ...
                <q-select :options="filteredExpedientes" map-options emit-value outlined v-model="formData.expedientes"
                  ...
                  hide-selected :readonly="expedienteInit" :disable="props.isDisabled || expedienteInit">
                  <template v-slot:no-option>
                    <q-item>
                      <q-item-section class="text-grey">
                        Sin resultados
    ...
    <div class="justify-center tw-w-full tw-pb-6 tw-mt-4 tw-flex tw-gap-4" v-if="!expedienteInit">
      <q-btn label="Cancelar" type="reset" style="width: 240px; height: 40px" text-color="black" color="accent"
        class="tw-rounded-xl" @click="() => (showModal = true)" :disable="props.isDisabled" />
      <q-btn label="Guardar" text-color="white" @click="submit" style="width: 240px; height: 40px" color="primary"
        class="tw-rounded-xl tw-btn tw-p-2" :disable="props.isDisabled" />
    </div>
  </div>

  <crearExpediente ref="crearComp" v-if="crearExp" :ml-data="props.mlData" @finally="emit('success', $event)"
...
import { Medicina, SelectInput } from "src/interfaces";
...
const listadoSeries: Ref<any> = ref([]);
...
const loadExpediente = async () => {
  const { data: expediente } = await sgdeaAxios.get("/expediente/children", {
    params: { nodeId: props.mlData.nodeId },
  });
...
watch(() => props.mlData, async (newValue) => {
  if (newValue) {
    await loadDependencias();
    await loadExpediente();
  }
...
const loadExpedientes = async (idSerie?: number) => {
  ...
  if (formData.value.subSeries) {
    params.idSubserie = formData.value.subSeries || formData.value.subSeries;
  } 
    ...
    try {
      const sendData = {
       ...
       dependencia: formData.value.dependencia.value,
        // @ts-expect-error
        serie: formData.value.series.value || formData.value.series,
        subserie: subseriesRequired.value ? formData.value.subSeries : null,
      ...
      if (response.status === 200 || response.status === 201) {
        await crearTrazabilidad({
          ...
          accion: `${auth.$state.userInfo.name} incluyó un expediente en el tramite ${props.mlData.idRadicado}`,
      ...
      if (error.response?.status === 500) {
        toast.error(error.response?.data?.message || "Ha ocurrido un error al guardar los datos");
      } else {
        toast.error(error.response?.data?.message || "Ha ocurrido un error al guardar los datos");
      }
    }
...
watch(() => formData.value.subSeries, (newValue) => {
  // @ts-expect-error
  loadTipoDocumental(newValue || newValue.value);
  loadExpedientes();
  if (!expedienteInit.value) {
    formData.value.tipoDocumental = "";
    formData.value.expedientes = "";
