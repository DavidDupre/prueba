  ...
  <label for="" class="grid tw-h-auto">
    ...
    <figure v-if="!value || value.length === 0" class="flex tw-items-center tw-justify-center !tw-text-sm">
      <div v-if="!dense">
        ...
        <p class="text-grey-7 tw-text-xs tw-text-center" v-if="maxFileSize">
          Puede adjuntar (30) Archivo(s)cada uno con un tamaño permitido de {{ `${maxFileSizeLabel}MB` ?? "2MB" }} en
          cualquiera de las siguiente extenciones (pdf, png, jpg, jpeg, webp, bmp, txt, doc, docx, xls, xlsx, mp3, mp4, avi, csv, zip, rar, ppt, pptx, eml, msg)
        </p>
      </div>
      <div v-else>
