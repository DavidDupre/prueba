<template>
  <div class="row justify-end">
    <q-btn color="secondary"  label="Agregar comentario" v-if="!['Aprobado', 'Por publicar', 'Devolucion', 'Publicado', 'Por enviar', 'Enviado'].includes(detailsRadicado.estado)" dense no-caps @click="showCommentModal" />
  </div>
...
async function loadData() {
  try {
    const response = await sgdeaAxios.get('/trazabilidad/obtenerPorCodProceso/' + 'ML' + props.detailsRadicado.id)
...
async function addComment() {
  try {
    await crearTrazabilidad({
      proceso: `ML${props.detailsRadicado.id}`,
