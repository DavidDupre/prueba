<template>
  <div class="row justify-end">
    <q-btn color="secondary" label="Agregar comentario" v-if="!['Aprobado', 'Por publicar', 'Devolucion', 'Publicado', 'Por enviar', 'Enviado'].includes(props.estado)" dense no-caps @click="showCommentModal" />
  </div>
      ...
      <q-card class="dialog tw-p-4">
        <q-card-section class="tw-bg-white">
          ...
          <span class="tw-w-full">
              <CommentTextArea v-model="comment"  :max-length="1000"
...
async function loadData() {
  try {
    const response = await sgdeaAxios.get('/trazabilidad/obtenerPorCodProceso/' + 'MLCOM' + props.idComunicacion)
...
async function addComment() {
  try {
    await crearTrazabilidad({
      proceso: `MLCOM${props.idComunicacion}`,
      secuencia: props.secuencia,
      estado: props.estado,
      descripcion: `Usuario: ${auth.userInfo.name} Comentario: ${comment.value}`,
      comentario: comment.value,
      nombre: auth.$state.userInfo.name,
      accion: `${auth.$state.userInfo.name} ha hecho un comentario en la comunicación`,
