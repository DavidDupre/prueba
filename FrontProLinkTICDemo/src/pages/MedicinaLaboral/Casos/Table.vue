<template>
    <div>
        <q-table flat :rows-per-page-options="[5, 25, 50, 100]" :rows="TABLE_BODY" :columns="TABLE_HEADER"
            :row-key="keyId" table-header-class="text-black" class="q-mr-md full-width" rows-per-page-label="Páginas"
            color="secondary" :selection="isSelection ? 'multiple' : 'none'" v-model:selected="selection">
            ...
            <template v-slot:body-cell-trazabilidad="props">
                <q-td class="tw-cursor-pointer">
                    <q-icon name="history" size="xs" @click="handleTrazabilidad" />
                </q-td>
            </template>
            ...
            <template v-slot:body-cell-acciones="props">
                <q-td class="text-center">
                    <q-btn size="sm" color="secondary" label="Asignar" @click="viewDetails(props.row[rowId])" />
            ...
            <template v-slot:no-data="{ icon, message }">
                <div class="full-width row flex-center tw-flex tw-flex-col tw-py-8">
                    ...
                    <span class="tw-text-lg tw-mt-4 tw-w-[480px] tw-text-center tw-leading-5">
                        {{ msgNoData ? msgNoData : `No se encontraron datos con los criterios ingresados. Por favor
