<template>
            ...
            <div class="tw-bg-white tw-p-4" style="border-radius: 5px">
              ...
              <div class="row tw-p-2">
                ...
                <div class="col-sm tw-px-2 tw-pt-2">
                  ...
                  <q-select v-model="formData.oficina" :disable="isOficinaSelectorDisabled" :options="filteredOficinas" emit-value label="Seleccione" dense
                    outlined class="tw-rounded-lg" use-input map-options @filter="selectOptionsFilterFn" fill-input
                    :rules="[]" hide-selected input-debounce="0">
              ...
              <div class="row tw-p-2">
                <div class="col-sm-12 tw-p-2" v-if="!showPqrdForm">
                  <p class="tw-text-label tw-font-semibold">Observaciones* {{ formData.descripcion.length }}/10000</p>
                  <q-input type="textarea" outlined label="Inserte" dense emit-value maxLengthInput="10000"
                    v-model="formData.descripcion" :rules="[
                      (v) => inputRequired(v),
                  ...
                  <q-expansion-item label="Solicitud" default-opened header-class="tw-text-lg tw-font-bold">
                    ...
                    <div class="tw-grid tw-grid-cols-6 tw-gap-x-4">
                      ...
                      <div class="tw-col-span-6" v-if="Number(formDataPqrd.tipoSolicitud) === 14">
                        <CommentTextArea v-model="formDataPqrd.solicitudDetalle" :is-required="true" :max-length="150"
                          label="Detalle" />
                      </div>
                      <div class="tw-col-span-6 tw-mb-5">
                        <CommentTextArea v-model="formDataPqrd.observaciones" :is-required="true" :max-length="1500"
  ...
  <Modal v-model:model-value="showModal" title="¡Espera un momento!"
    ...
    :is-success="false" textButtonCancel="No" cancel-button @handleAccept="
      showModal = false; resetForm();" text-button="Si" text-button-cancel="No" @close-modal="() => (showModal = false)" />
...
async function getTiposPQRD() {
  try {
    ...
    // Aquí defines los IDs que deseas ocultar (vacío por ahora)
    const idsParaOcultar = [2, 3, 5, 6, 7, 9, 10, 11, 12];

    // Filtramos la data para excluir IDs específicos y ordenamos los resultados
    tiposPQRD.value = data
      .filter(tramite => !idsParaOcultar.includes(tramite.id)) // Filtra según IDs que quieres ocultar
      .sort((a, b) => a.nombreTramite.localeCompare(b.nombreTramite)) // Ordena alfabéticamente por 'nombreTramite'
      .map(({ nombreTramite, id }) => {
        // Mapeamos para obtener el formato que necesitamos
        return {
          label: nombreTramite,
          value: id
        };
    ...
    // Capturamos y mostramos errores si ocurren
    console.error(error);
  }
}
...
async function save() {
  ...
  if (showPqrdForm.value) {
    ...
    dataPqrd = {
      solicitud: {
        ...
        idCasoOrigen: props.detalleTutela.idTutela,
        usuarioSolicitanteId: auth.$state.userInfo.userid,
        observaciones: formData.value.descripcion || formDataPqrd.value.observaciones,
  ...
  try {
    ...
    if (showPqrdForm.value) {
      ...
      response = await sgdeaAxios.post('/tutela/reclasificar', data);
    }
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      //   estado: props.detalleTutela.estado.estado,
      //   descripcion: "Se ha hecho una solicitud de reclasificación de la Tutela",
      //   comentario: 'Reclasificar',
      //   nombre: auth.$state.userInfo.name,
      //   accion: `${auth.$state.userInfo.name} solicitó una reclasificación de la Tutela`,
      //   // tramite: props.detalleTutela.informacionSolicitud.tipologiaEntity.id,
      ...
      toast.error(response.data.message || 'Error desconocido al procesar el trámite.');
    }
  } catch (error) {
    const errorMessage = error.response?.data?.message || 'Ha ocurrido un error.';
    toast.error(errorMessage);// Mantener el log para depuración
  }
}
interface PQRD {
...
const handleDownloadRadicacionDocument = async () => {
  try {
    const { data: listDocs } = await intescoAxios.get<any>(
      `/bandeja/consultarAnexosPorCarpeta?nodeId=${generatedDocId.value}`
    );

    const docRadicado = listDocs.result.radicado?.find((it: { node: { entry: { name: any; }; }; }) => {
      const entryName = it.node.entry.name
      return entryName && entryName.startsWith(responseId.value)
    })

    const { data } = await sgdeaAxios.get<DocumentoRadicado>(
      `/radicados/consultar_documento/${docRadicado.node.entry.id}`
    );
    ...
    toast.error("Error al descargar el documento de radicación.");
  }
};
