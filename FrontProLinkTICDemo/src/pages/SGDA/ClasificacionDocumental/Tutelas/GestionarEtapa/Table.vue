    ...
    <q-table flat :rows="TABLE_BODY" :columns="TABLE_HEADER" :row-key="keyId" table-header-class="text-black text-h6 "
      ...
      <template v-slot:body-cell-acciones="props">
        <q-td class="text-center">
          <q-btn size="sm" color="secondary" :icon="icon || 'edit'" flat @click="
            ...
            " />
          <q-btn size="sm" color="secondary" :icon="secondIcon" flat
      ...
      <template v-slot:body-cell-actions="props">
        <q-td class="text-center">
          ...
          <q-btn size="sm" color="primary" :icon="secondIcon" flat v-if="props.row.formato.toLowerCase() == 'pdf'"
            @click="handleSecondIcon(props.row.nodeId, TABLE_BODY.filter((it) => obtenerExtension(it.nombre).toLowerCase() == 'pdf'))" />
        </q-td>
      </template>
