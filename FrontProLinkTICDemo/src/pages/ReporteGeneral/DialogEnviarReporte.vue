      ...
      <q-card class="tw-px-10">
        <q-card-section class="tw-bg-white">
          <div class="full-width row tw-justify-center tw-items-center">
            <img :src="require('src/assets/mail.svg')" alt="email" />
          </div>
        </q-card-section>
        ...
        <q-card-section>
          ...
          <div class="tw-mt-6">
            ...
            <q-select v-model="inputFormat" :rules="[inputSelectRequired]" outlined map-options emit-value
            ...
            <q-item class="tw-border-b">
              ...
              <q-item-section>
                <q-select v-model="correo" input-debounce="0" label="Inserte el correo" borderless dense
                  ...
                  @input-value="(val) => val != correo ? correo = val : correo" :rules="[validationEmail]"
                  class="tw-mb-[-20px]" />
              </q-item-section>
          ...
          <q-scroll-area style="height: 100px; width:100%;">
            <div v-for="(_item, index) in selectedCorreos" :key="index">
              <q-item class="tw-border-b">
                <q-item-section avatar class="tw-flex tw-flex-row tw-gap-2 tw-justify-center tw-items-center">
                  <q-icon name="check_circle_outline" size="sm" color="grey-6" />
                  <span>{{ index + 1 }}</span>
                </q-item-section>
...
const props = defineProps<{ open: boolean, dataTable: any[], tipoReporte: Number }>()
...
watchEffect(() => {
  if (props.dataTable) {
    ...
    if (props.tipoReporte == 1) {
      data = [...new Set(props.dataTable.map((v: any) => {
        return v.direccionCorreoElectronico?.toLowerCase()
      }))];
    }
    else if (props.tipoReporte == 2) {
      data = [
        ...new Set(props.dataTable.map((v: any) => {
          return v.correoRemitente?.toLowerCase()
        })),
        ...new Set(props.dataTable.map((v: any) => {
          return v.correoElectronicoDeEnvio?.toLowerCase()
        }))
      ];
    }
    else if (props.tipoReporte == 3) {
      data = [...new Set(props.dataTable.map((v: any) => {
        return v.correoElectronicoDestinatario?.toLowerCase()
      }))];
    }
    else if (props.tipoReporte == 6) {
      data = [
        ...new Set(props.dataTable.map((v: any) => {
          return v.correoDelDestinatario?.toLowerCase()
        })),
        ...new Set(props.dataTable.map((v: any) => {
          return v.correoDelDestinatarioDevolucion?.toLowerCase()
        }))
      ];
    }
    else if (props.tipoReporte == 7) {
      data = [...new Set(props.dataTable.map((v: any) => {
        return v.correoDestinatario?.toLowerCase()
      }))];
    }

    data = data.filter(correo => correo && correo.trim() !== "");
