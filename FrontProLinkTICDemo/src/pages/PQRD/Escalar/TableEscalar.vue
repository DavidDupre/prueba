<template>
  <div>
    <q-table flat :rows="TABLE_BODY" :columns="TABLE_HEADER" :row-key="rowKey" table-header-class="text-black text-h6 "
      class="sizes q-mr-md tableclass" style="margin: 0 auto; border-radius: 15px; min-height: auto; max-height: 400px "
      :filter="filterValue" rows-per-page-label="Páginas" :selection="isSelection ? 'multiple' : typeSelection"
      v-model:selected="selection" virtual-scroll :rows-per-page-options="[8]"@row-dblclick="handleDoubleClick" color="primary" >
      ...
      <template v-slot:top-left>
        ...
        <q-select v-if="canSearch" :options="filteredOficinas" hide-selected fill-input input-debounce="0" use-input
          v-model="formData.Oficinas" class="tw-w-[60%]" @filter="selectOptionsFilterFn" outlined dense
...
const selection = ref([])
const inputText = ref('')
const emit = defineEmits(['update:itemSelected', 'update:officeSelected','update:dragEvent', 'update:handleDblClickEvent','update:mySelection'])
