...
const loadExpediente = async () => {
  const { data: expediente } = await sgdeaAxios.get("/expediente/children", {
    // @ts-ignore
    params: { nodeId: props.pqrdData.nodeID },
  });
...
const isCrearExpediente = () => {
  // Lógica para manejar la creación de expediente
}
  ...
  try {
    ...
    if (status === 200) {
      await crearTrazabilidad({
        ...
        accion: `${auth.$state.userInfo.name} ha incluido un expediente en el trámite ${props.pqrdData.idRadicado}`,
        tramite: "PQRD",
      });
...
// Watch modificado para subseries
watch(
  () => formData.value.subSeries,
  (newValue) => {
    if (newValue) {
