          ...
          <div class="tw-grid tw-grid-cols-4 tw-gap-x-4">
            ...
            <div class="tw-col-span-2 tw-mt-4">
              ...
              <q-input type="text" v-model="formData.numeroDocumento" label="Inserte" dense outlined
                :rules="[inputRequired, v => maxLengthInput(30, v)]" @keypress="onlyAlphaNumeric" />
            </div>
            <div class="tw-col-span-2 tw-mt-4">
...
// Nuevas variables para la lógica de subseries opcional
...
onMounted(async () => {
  ...
  formData.value.numeroDocumento = props.pqrdData.informacionAfectada.numeroDocumentoAfectado;
  formData.value.numeroIdentificacion = props.pqrdData.casoIdBonita;
  formData.value.expedientes = `EXP-${props.pqrdData.casoIdBonita}-${formData.value.numeroDocumento == 'N/A' ? 'NA' : formData.value.numeroDocumento}`
  await getData()
});
...
const submit = async () => {
  ...
  formData.value.expedientes = `EXP-${formData.value.numeroIdentificacion}-${formData.value.numeroDocumento}`;
  ...
  try {
    const sendData = {
      ...
      creacion: true,
      nodeIdRad: props.pqrdData.nodeID,
      modulo: 1,
      radicado: props.pqrdData.idRadicado,
    }
  ...
  const tipo = listadoTiposDocumentales.value.find(i=>i.value === formData.value.tipoDocumental )?.label || '';
    ...
    if (status === 200) {
      ...
      await crearTrazabilidad({
        ...
        accion: `${auth.$state.userInfo.name} ha creado e incluido un expediente en el tramite ${props.pqrdData.idRadicado}`,
        tramite: "PQRD",
      });
    }
