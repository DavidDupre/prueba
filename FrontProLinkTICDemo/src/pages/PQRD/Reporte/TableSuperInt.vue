    ...
    <div>
        <q-table flat :rows="TABLE_BODY" :columns="TABLE_HEADER" :row-key="keyId" table-header-class="text-black text-h6"
            ...
            <template v-slot:body-cell-acciones="props">
                <q-td class="text-center">
                    <q-btn class="tw-w-auto tw-h-9 " @click="editData(props.row.id, false)">
                    ...
                    <q-btn class="tw-w-auto tw-h-9 tw-ml-2" icon="plagiarism" @click="editData(props.row.id, true)">
                    </q-btn>
                </q-td>
            </template>
...
const editData = (id: number, validDoc) => {
    emit('handleEditData', id, validDoc)
}
