...
<template>
  <div>
    <q-table flat :rows="props.TABLE_BODY" :columns="props.TABLE_HEADER" :row-key="props.keyId" table-header-class="text-black text-h6 " class="q-mr-md full-width tw-bg-transparent" rows-per-page-label="Páginas" hide-pagination
      v-model:pagination="pagination" :selection="isSelection" :loading="loading" v-model:selected="selection">

      <template v-slot:body-cell-actions="props">
        <q-td class="text-center">
          <q-btn size="sm" color="primary" :icon="icon" flat @click="editData(props.row)" :disable="isDeleteDisable"/>
        </q-td>
      </template>

      <template v-if="!loading" v-slot:no-data="{ icon, message, filter }">
        <div class="full-width row flex-center tw-flex tw-flex-col tw-py-8">
          <img v-lazy :data-url="require('assets/filterImage.svg')" alt="" width="120" />
