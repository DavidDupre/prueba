        ...
        <section class="tw-px-8 tw-pt-3">
          <div class="tw-grid tw-grid-cols-4 tw-gap-x-4">
            ...
            <div class="tw-col-span-2 tw-mt-4">
              ...
              <q-input type="text" v-model="formData.numeroIdentificacion" label="Inserte" dense outlined :rules="[
                inputRequired,
                onlyNumbers,
                (v) => maxLengthInput(10, v),
              ]" @keypress="onlyAlphaNumeric" readonly />
            </div>
            <div class="tw-col-span-2 tw-mt-4">
              <p class="tw-text-label tw-font-semibold">Número de Identificación *</p>
              <q-input type="text" v-model="formData.numeroDocumento" label="Inserte" dense outlined :rules="[
                inputRequired,
                noCaracteresEspecialesSelect,
                (v) => maxLengthInput(30, v),
              ]" @keypress="onlyAlphaNumeric" readonly />
            </div>
            <div class="tw-col-span-2 tw-mt-4">
...
const emit = defineEmits(["crearExpediente"]);
...
onMounted(async () => {
  ...
  formData.value.numeroDocumento = props.informacionGeneral?.informacionAfectado?.numeroDocumentoAfectado;
  // @ts-expect-error
  formData.value.numeroIdentificacion = numberId;
  formData.value.expedientes = `EXP-${numberId}-${formData.value.numeroDocumento == 'N/A' ? 'NA' : formData.value.numeroDocumento}`
  await getData();
});

const getRandomNumber = () => {
  return Math.floor(Math.random() * (25000 - 20001) + 20001);
};
  ...
  try {
    ...
    listadoSeries.value = responseSeries.data.map((item) => {
      return {
        ...
        fullData: item // Guardamos toda la información de la serie
      }
    })
  } catch (error) {
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      await crearTrazabilidad({
       ...
       descripcion:
        ...
        accion: `${auth.$state.userInfo.name} ha creado e incluido un expediente al trámite ${props.informacionGeneral.nroRadicado}`,
        tramite: "Entes de Control",
      });
      emit("crearExpediente", response.data.nodeId);
    }
...
// Watch modificado para usar handleSeriesChange
